...
// Memoized banner content to prevent re-renders
const BannerContent = memo(({ isExpired, balance }: { isExpired: boolean; balance?: number }) => (
    <>
            ...
            <div>
                ...
                <p className="text-sm font-bold opacity-95">
                    {isExpired
                        ...
                        : `Your balance (${balance} EGP) is below requirements. Recharge to keep your store public.`}
                </p>
            </div>
        </div>
...
export const BillingBanner = memo(() => {
    ...
    return (
        <div className={cn(
            ...
            <BannerContent isExpired={isExpired} balance={billing.wallet.balance} />
        </div>
    );
});
