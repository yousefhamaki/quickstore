...
import { getEmailAccountBalance, buyEmailAddOn, transferEmailCredits } from "@shared/services/marketingService";
...
export default function MerchantBillingPage() {
    ...
    const handleTransferCredits = async () => {
        ...
        if (!transferToStoreId || !amount || amount <= 0) {
            toast.error('Pick a destination store and a valid amount');
            return;
        }
        try {
            setTransferring(true);
            const res = await transferEmailCredits(selectedStoreId, transferToStoreId, amount);
    ...
    return (
        <div className="container mx-auto p-6 max-w-7xl space-y-8 pb-20">
                    ...
                    <CardContent className="space-y-6">
                        ...
                        <div className="space-y-4 pt-4 border-t border-white/10">
                            <Button
                                ...
                                className="bg-white text-primary hover:bg-white/90 rounded-xl font-black h-11 px-6 transition shadow-lg w-full"
                            >
                            ...
                            <div className="space-y-4 flex-1 flex flex-col justify-end">
                                ...
                                {stores && stores.length > 1 && (
                                    <Button
                                        ...
                                        disabled={!emailBalance.purchasedBalance}
                                        variant="outline"
                                        className="w-full rounded-xl font-black h-11 border-2"
                                    >
                                        <ArrowLeftRight className="w-4 h-4 mr-2" /> {t('transferCredits')}
                                    </Button>
                                )}
                            </div>
                        ) : (
                                        ...
                                        <tr key={tx._id} className="hover:bg-muted/20 transition-colors">
                                            ...
                                            <td className="px-6 py-5 text-right">
                                                <Button
                                                    variant="ghost"
                                                    size="icon"
                                                    className="h-9 w-9 rounded-xl hover:bg-primary hover:text-white transition shadow-sm"
            ...
            {/* Transfer Email Credits Dialog */}
            <Dialog open={transferOpen} onOpenChange={setTransferOpen}>
                <DialogContent className="rounded-3xl max-w-md border-2">
                    <DialogHeader className="space-y-2">
                        <DialogTitle className="text-xl font-black uppercase tracking-tight flex items-center gap-2">
                            <ArrowLeftRight className="w-5 h-5 text-indigo-600" /> {t('transferCredits')}
                        </DialogTitle>
                        <DialogDescription className="font-medium text-xs">
                            {t('transferCreditsSubtitle', { store: stores?.find((s: any) => s._id === selectedStoreId)?.name || '' })}
                        </DialogDescription>
                    </DialogHeader>

                    <div className="space-y-4 my-2">
                        <div className="space-y-1.5">
                            <label className="text-[10px] font-black uppercase tracking-wider text-muted-foreground">{t('transferTo')}</label>
                            <select
                                value={transferToStoreId}
                                onChange={(e) => setTransferToStoreId(e.target.value)}
                                className="w-full bg-background border-2 rounded-xl px-3 py-2 text-sm font-bold focus:outline-none focus:border-primary"
                            >
                                <option value="">{t('selectStore')}</option>
                                {stores?.filter((s: any) => s._id !== selectedStoreId).map((store: any) => (
                        ...
                        <div className="space-y-1.5">
                            <label className="text-[10px] font-black uppercase tracking-wider text-muted-foreground">
                                {t('transferAmount', { available: emailBalance?.purchasedBalance ?? 0 })}
                            </label>
                            <Input
                                type="number"
                                min={1}
                                max={emailBalance?.purchasedBalance ?? 0}
                    ...
                    <DialogFooter className="gap-2">
                        <Button type="button" variant="ghost" onClick={() => setTransferOpen(false)} className="rounded-xl font-bold uppercase tracking-wider text-[9px] h-10">
                            {locale === 'ar' ? "إلغاء" : "Cancel"}
                        </Button>
                        <Button
                            onClick={handleTransferCredits}
                            disabled={transferring || !transferToStoreId || !transferAmount}
                            className="rounded-xl font-black uppercase tracking-wider text-[9px] h-10 bg-indigo-600 hover:bg-indigo-700"
                        >
                            {transferring ? <Loader2 className="w-4 h-4 animate-spin" /> : t('transferCredits')}
                        </Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
