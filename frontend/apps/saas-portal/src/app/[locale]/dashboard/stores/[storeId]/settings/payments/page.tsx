                        ...
                        <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                            {[
                                ...
                                return (
                                    <div
                                        key={prov.id}
                                        onClick={() => prov.available && setValue("payment.provider", prov.id, { shouldDirty: true })}
                                        title={prov.available ? undefined : 'Coming soon — not available yet'}
                                        aria-disabled={!prov.available}
                                        className={cn(
                                            "relative p-4 rounded-xl border-2 transition flex flex-col items-center justify-center text-center gap-2",
                                            prov.available ? "cursor-pointer" : "cursor-not-allowed opacity-50 grayscale",
                                            isActive ? `ring-2 ring-primary/20 shadow-md ${prov.color}` : "border-border bg-background",
                                            prov.available && !isActive && "hover:border-primary/40"
                                        )}
                                    >
                                        {!prov.available && (
                                            <span className="absolute top-1 right-1 text-[8px] font-black uppercase tracking-wide bg-muted text-muted-foreground rounded-full px-1.5 py-0.5">
                                                Soon
                                            </span>
                                        )}
                        ...
                        {watch("payment.provider") === 'paymob' && (
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-in slide-in-from-top-4 fade-in duration-300 pt-6 border-t border-primary/10 mt-6">
                                <div className="space-y-2">
                                    <Label>API Key</Label>
                                    <PasswordInput placeholder={store?.settings?.payment?.credentials?.apiKey ? "••••••••••••" : "Enter API Key"} {...register("payment.credentials.apiKey")} />
                                </div>
                                <div className="space-y-2">
                                    <Label>HMAC Secret</Label>
                                    <PasswordInput placeholder={store?.settings?.payment?.credentials?.apiSecret ? "••••••••••••" : "Enter HMAC Secret"} {...register("payment.credentials.apiSecret")} />
                                </div>
                                <div className="space-y-2">
                        ...
                        {watch("payment.provider") === 'stripe' && (
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-in slide-in-from-top-4 fade-in duration-300 pt-6 border-t border-primary/10 mt-6">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <PasswordInput placeholder={store?.settings?.payment?.credentials?.apiKey ? "sk_live_••••••••" : "sk_live_..."} {...register("payment.credentials.apiKey")} />
                                </div>
                                <div className="space-y-2 md:col-span-2">
                                    <Label>Webhook Signing Secret</Label>
                                    <PasswordInput placeholder={store?.settings?.payment?.credentials?.apiSecret ? "whsec_••••••••••••" : "whsec_..."} {...register("payment.credentials.apiSecret")} />
                                </div>
                                <p className="text-[11px] font-medium text-muted-foreground md:col-span-2">All gateway keys are securely encrypted in our databases using AES-GCM algorithms before saving.</p>
                            </div>
                        )}
                        ...
                        {watch("payment.provider") === 'paypal' && (
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-in slide-in-from-top-4 fade-in duration-300 pt-6 border-t border-primary/10 mt-6">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <PasswordInput placeholder={store?.settings?.payment?.credentials?.apiSecret ? "••••••••••••" : "Enter Application Secret"} {...register("payment.credentials.apiSecret")} />
                                </div>
                                <p className="text-[11px] font-medium text-muted-foreground md:col-span-2">All gateway keys are securely encrypted in our databases using AES-GCM algorithms before saving.</p>
                            </div>
                        )}
                        ...
                        {watch("payment.provider") === 'fawry' && (
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-in slide-in-from-top-4 fade-in duration-300 pt-6 border-t border-primary/10 mt-6">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <PasswordInput placeholder={store?.settings?.payment?.credentials?.apiSecret ? "••••••••••••" : "Enter Secret Verification Key"} {...register("payment.credentials.apiSecret")} />
                                </div>
                                <p className="text-[11px] font-medium text-muted-foreground md:col-span-2">All gateway keys are securely encrypted in our databases using AES-GCM algorithms before saving.</p>
                            </div>
                        )}
