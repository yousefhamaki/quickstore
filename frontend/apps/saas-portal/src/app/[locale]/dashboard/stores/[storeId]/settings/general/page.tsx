...
export default function GeneralSettings({ params }: { params: Promise<{ storeId: string }> }) {
    ...
    const [emailCreditsPrompt, setEmailCreditsPrompt] = useState<{ purchasedBalance: number; otherStores: { _id: string; name: string }[] } | null>(null);
    const [transferToStoreId, setTransferToStoreId] = useState('');

    const resetDeleteDialog = () => {
        setConfirmPassword('');
        setEmailCreditsPrompt(null);
    ...
    const handleDeleteStore = async () => {
        try {
            await deleteMutation.mutateAsync({
                ...
                ...(emailCreditsPrompt ? { transferCreditsToStoreId: transferToStoreId } : {})
            });
            setIsDeleteDialogOpen(false);
            router.push('/dashboard');
        } catch (err: any) {
            if (err?.response?.data?.code === 'EMAIL_CREDITS_TRANSFER_REQUIRED') {
                // Store has purchased email credits and other stores exist to
                // move them to — switch the dialog to ask which one instead
                // of failing outright.
                setEmailCreditsPrompt({
                    purchasedBalance: err.response.data.purchasedBalance,
                    otherStores: err.response.data.otherStores
                });
            }
            // Any other error toast is handled by the hook.
        }
    };
                    ...
                    <CardContent className="pt-6 space-y-8">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div className="space-y-4">
                                <div className="space-y-2">
                                    ...
                                    <div className="flex items-center gap-4">
                                        {logoValue?.url ? (
                                            <div className="relative w-20 h-20 rounded-2xl border-2 overflow-hidden bg-muted flex items-center justify-center group shadow-sm transition hover:border-red-500">
                                                <img src={imagePreset.logo(logoValue.url)} alt="Store Logo" loading="lazy" decoding="async" className="w-full h-full object-contain p-1" />
                                                <button
                                                    type="button"
                                            ...
                                            <label className="w-20 h-20 rounded-2xl border-2 border-dashed flex flex-col items-center justify-center cursor-pointer hover:bg-muted/50 hover:border-primary transition bg-muted/20">
                                                {isUploading ? (
                            ...
                            <div className="space-y-4">
                                ...
                                <div
                                    className="h-40 rounded-3xl border-4 border-dashed p-6 flex flex-col justify-center items-start gap-2 transition relative overflow-hidden bg-white"
                                    style={{ borderColor: watch("branding.primaryColor"), fontFamily: watch("branding.fontFamily") }}
                                >
                                    {logoValue?.url ? (
                                        <img src={imagePreset.logo(logoValue.url)} alt="Preview Logo" loading="lazy" decoding="async" className="h-8 w-auto object-contain mb-1" />
                                    ) : (
            ...
            <Dialog open={isDeleteDialogOpen} onOpenChange={(open) => {
                ...
                <DialogContent>
                    {!emailCreditsPrompt ? (
                        ...
                        <>
                            <DialogHeader>
                                <DialogTitle className="text-2xl font-black text-amber-600">Move Email Credits First</DialogTitle>
                                <DialogDescription className="pt-4 text-base space-y-4">
                                    <span className="block bg-amber-50 border-2 border-amber-200 text-amber-900 p-6 rounded-2xl font-medium leading-relaxed">
                                        This store has <strong>{emailCreditsPrompt.purchasedBalance}</strong> purchased email credits
                                        (not part of your plan's monthly allowance — credits you paid for). Choose another one of
                                        your stores to move them to before this store is deleted, or they'll be lost.
                                    </span>
                                </DialogDescription>
                            </DialogHeader>
                            <div className="my-4 space-y-1.5">
                                <Label className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Move credits to</Label>
                                <select
                                    value={transferToStoreId}
                                    onChange={(e) => setTransferToStoreId(e.target.value)}
                                    className="w-full h-11 rounded-xl border-2 bg-background px-3 text-sm font-bold focus:outline-none focus:border-primary"
                                >
                                    <option value="">Select a store</option>
                                    {emailCreditsPrompt.otherStores.map((s) => (
                            ...
                            <DialogFooter className="flex gap-3 sm:justify-end">
                                ...
                                <Button
                                    ...
                                    className="rounded-xl font-black h-11 px-6 shadow-xl shadow-red-500/20"
                                    onClick={handleDeleteStore}
                                    disabled={!transferToStoreId || deleteMutation.isPending}
                                >
                                    {deleteMutation.isPending ? <Loader2 className="w-5 h-5 animate-spin mr-2" /> : <Trash2 className="w-5 h-5 mr-2" />}
                                    Move Credits & Delete Store
                                </Button>
                            </DialogFooter>
                        </>
                    )}
