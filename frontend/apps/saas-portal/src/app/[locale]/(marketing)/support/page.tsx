...
import { SupportFAQSection } from '@shared/components/support/SupportFAQSection';
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                        ...
                        <ChannelCard
                            ...
                            color="green"
                            onClick={() => window.dispatchEvent(new Event('buildora:open-chat'))}
                        />
                        <ChannelCard
            ...
            <section className="py-20 md:py-32 bg-white relative overflow-hidden">
                <div className="max-w-4xl mx-auto px-4 text-center">
                    ...
                    <Link href="/contact">
                        <Button
                            size="lg"
                            className="h-16 px-12 text-xl rounded-full bg-gray-900 hover:bg-gray-800 text-white font-bold transition hover:scale-105"
                        >
...
function ChannelCard({ icon, title, description, buttonText, color, href, onClick }: { icon: React.ReactNode, title: string, description: string, buttonText: string, color: 'blue' | 'green' | 'orange', href?: string, onClick?: () => void }) {
    const colorMap = {
    ...
    const button = (
        <Button variant="outline" className="w-full h-12 rounded-xl border-gray-200 font-bold hover:bg-gray-50" disabled={!href && !onClick} onClick={onClick}>
            {buttonText} <ExternalLink className="ml-2 h-4 w-4 rtl:mr-2 rtl:ml-0" />
        </Button>
    );

    return (
        <div className={`p-10 rounded-3xl bg-white border-2 border-gray-50 transition duration-300 hover:shadow-2xl hover:-translate-y-2 text-center ${colorMap[color]}`}>
