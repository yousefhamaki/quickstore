...
import { getStoreProducts } from '@shared/services/publicStoreService';
...
export default async function sitemap(
    ...
    try {
        ...
        const sitemapEnabled = (store as any).seo?.sitemapEnabled !== false &&
            (store.settings?.marketing as any)?.sitemapEnabled !== false;
        ...
        const products = await getStoreProducts((store as any)._id) as any[];
