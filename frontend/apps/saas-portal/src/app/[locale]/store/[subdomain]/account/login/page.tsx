...
export default function AccountLoginPage() {
    const t = useTranslations('store.account.login');
                    ...
                    <form onSubmit={handleSubmit} className="space-y-5">
                        ...
                        <div className="space-y-2">
                            <div className="flex items-center justify-between ml-1">
                                <label className="text-[10px] font-black uppercase tracking-widest">{t('password')}</label>
                                <Link href="/account/forgot-password" className="text-[10px] font-bold text-gray-400 hover:text-black transition-colors">
                                    {t('forgotLink')}
                                </Link>
                            </div>
                            <PasswordInput
                        ...
                        >
                            {isSubmitting ? <Loader2 className="w-5 h-5 animate-spin" /> : <><LogIn className="w-4 h-4 mr-2" /> {t('submit')}</>}
                        </Button>
                    </form>

                    <p className="text-center text-sm text-gray-500 font-medium">
                        {t('noAccount')}{' '}
                        <Link href="/account/register" className="font-black" style={{ color: primaryColor }}>
                            {t('createOne')}
                        </Link>
                    </p>
                </CardContent>
