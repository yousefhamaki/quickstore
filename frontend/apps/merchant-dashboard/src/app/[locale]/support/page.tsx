...
export default function SupportPage() {
    ...
    const focusedArticle = articleId ? articles.find(a => a._id === articleId) : null;
                        ...
                        {loading ? (
                            ...
                            <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                                <div className="p-10 md:p-14 rounded-[40px] bg-white border-2 border-green-50 shadow-2xl shadow-green-100">
                                    <Badge className="mb-6 bg-green-50 text-green-600 px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-wider">
                                        {locale === 'ar' ? 'المقال المختار' : 'Selected Article'}
                                    </Badge>
                                    ...
                                    <div className="mt-12 pt-8 border-t border-gray-100 flex flex-wrap gap-4">
                                        ...
                                        <Link href="/contact">
                                            <Button className="rounded-full px-8 h-12 bg-green-600 hover:bg-green-700 text-white font-bold shadow-lg shadow-green-100">
                                                {locale === 'ar' ? 'ما زلت بحاجة للمساعدة؟ اتصل بنا' : 'Still need help? Contact Us'}
                                            </Button>
                                        </Link>
                                    </div>
                                </div>
                            ...
                            <div className="grid grid-cols-1 gap-6">
                                {articles.map(article => (
                                    <SupportFAQItem 
                                        key={article._id} 
                                        question={locale === 'ar' ? (article.titleAr || article.title) : article.title} 
                                        answer={locale === 'ar' ? (article.contentAr || article.content) : article.content} 
                                        id={article._id} 
                                        locale={locale}
                                    />
                                ))}
...
function SupportFAQItem({ question, answer, id, locale }: { question: string, answer: string, id?: string, locale?: string }) {
    return (
        <div className="p-8 rounded-3xl bg-white border border-gray-100 hover:shadow-lg transition-all text-left rtl:text-right group">
            <h3 className="text-xl font-black text-gray-900 mb-4">{question}</h3>
            <p className="text-gray-600 font-medium leading-relaxed mb-4">{answer.substring(0, 200)}{answer.length > 200 ? '...' : ''}</p>
            {id && (
                <Link href={`/support?id=${id}`} className="text-blue-600 font-bold text-sm inline-flex items-center hover:text-blue-800 transition-colors">
                    {locale === 'ar' ? 'اقرأ المزيد' : 'Read More'} <ArrowRight className={`ml-1 h-4 w-4 ${locale === 'ar' ? 'mr-1 rotate-180' : ''}`} />
                </Link>
            )}
        </div>
    );
