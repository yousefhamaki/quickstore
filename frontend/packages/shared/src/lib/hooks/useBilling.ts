...
export const useRechargeWallet = () => {
    ...
    return useMutation({
        mutationFn: ({ amount, method, walletNumber }: { amount: number; method: string; walletNumber?: string }) => 
            billingApi.rechargeWallet(amount, method, walletNumber),
        onSuccess: (data: any) => {
            if (data.paymentUrl) {
                window.location.href = data.paymentUrl;
            } else if (data.success && !data.referenceCode) {
