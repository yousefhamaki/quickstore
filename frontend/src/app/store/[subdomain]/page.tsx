...
export default async function StoreHome({ params }: { params: Promise<{ subdomain: string }> }) {
        ...
        <div className="space-y-20">
            ...
            <section className="container mx-auto px-4 py-20 animate-in fade-in duration-1000 delay-300">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-8 mb-12">
                    <h2 className="text-3xl font-black tracking-tighter">Shop Our Collection</h2>
                    <div className="flex flex-wrap gap-2">
                        {['All', ...new Set(products.map(p => p.category))].map((cat: any) => (
                            <button
                                key={cat}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                    {products.length === 0 ? (
                        <div className="col-span-full py-20 text-center space-y-4">
                            ...
                            <p className="text-gray-500 font-bold uppercase tracking-widest text-xs">Our catalog is currently being updated.</p>
                        </div>
                    ) : (
                            ...
                            >
                                <div className="aspect-[4/5] bg-gray-100 rounded-3xl overflow-hidden relative border shadow-sm group-hover:shadow-xl transition-all duration-500">
                                    ...
                                    <div className="absolute bottom-4 left-4 right-4 translate-y-4 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-300">
                                        <button className="w-full bg-white py-3 rounded-2xl font-black text-xs uppercase tracking-widest flex items-center justify-center gap-2 shadow-xl hover:bg-black hover:text-white transition-colors">
