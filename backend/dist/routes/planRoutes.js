...
router.get('/', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        try {
            ...
            cachedPlans = yield redisClient.get(cacheKey);
        }
        catch (redisErr) {
        ...
        const plans = yield SubscriptionPlan_1.default.find({ isActive: true });
        try {
            const { redisClient } = yield Promise.resolve().then(() => __importStar(require('../config/redis')));
            yield redisClient.setex(cacheKey, 86400, JSON.stringify(plans)); // 24 hour cache
        }
        catch (redisErr) {
